...
equivalentMetaObjects' equivs (MOSLType type1 tybs1) (MOSLType type2 tybs2) =
	equivalentSLTypesWithBindings equivs (tybs1, tybs2) type1 type2
equivalentMetaObjects' equivs (MOSLTerm term1 tybs1 tebs1) (MOSLTerm term2 tybs2 tebs2) =
	equivalentSLTermsWithBindings equivs (tybs1, tybs2) (tebs1, tebs2) term1 [] term2 []
...
equivalentSLTerms tlEquivs (tybs1, tybs2) (tebs1, tebs2) slEquivs
	...
	| name1 `M.member` tebs1 && name2 `M.member` tebs2 = let
		...
		tlNameSupply = filter (`S.notMember` tlNamesTaken) [Name ("n" ++ show i) | i <- [1..]]
		...
		subs1 = M.fromList [(paramName, MOSLTerm arg tybs1 (tebs1 ++ tebs1'))
		                   | ((paramName, _), arg) <- zip params1 args1]
		subs2 = M.fromList [(paramName, MOSLTerm arg tybs2 (tebs2 ++ tebs2'))
		...
		in length argsLeft1 == length argsLeft2
		   && and (zipWith
		               ...
		               argsLeft1 argsLeft2)
		   && equivalentMetaObjects' (tlEquivs ++ tlEquivs')
...
equivalentSLTerms tlEquivs tybs tebs slEquivs term1 args1@(_:_) term2 args2@(_:_) =
	...
	&& all (zipWith (\ t1 t2 -> equivalentSLTerms tlEquivs tybs tegs slEquivs t1 [] t2 []))
equivalentSLTerms tlEquivs tybs tebs slEquivs
			(SLR.TermDefined defn1 typs1) [] (SLR.TermDefined defn2 typs2) [] =
	SLR.nameOfTermDefn defn1 == SLR.nameOfTermDefn defn2
	&& all (zipWith (equivalentSLTypes tlEquivs tybs) typs1 typs2)
equivalentSLTerms tlEquivs tybs tebs slEquivs
...
equivalentSLTerms tlEquivs tybs tebs slEquivs
	...
	&& all (zipWith (equivalentSLTypes tlEquivs tybs) typs1 typs2)
	&& all (zipWith (\ t1 t2 -> equivalentSLTerms tlEquivs tybs tebs slEquivs t1 [] t2 []) teps1 teps2)
equivalentSLTerms tlEquivs tybs tebs slEquivs
