...
(let EitherAsPairLeft
	...
	(spec (sl-term "Left l r . x" (type "l" = lSL) (type "r" = rSL) (type "x" = xSL)))
...
(let EitherAsPairRight
		...
		= (js-expr
	(type (EitherAsPair lSL lJS rSL rJS))
	(spec (sl-term "Right l r . x" (type "l" = lSL) (type "r" = rSL) (type "x" = xSL)))
...
(let EitherAsPairCase
		...
		(subjectJS :: js-expr (EitherAsPair lSL lJS rSL rJS) sSL)
		(resTypeSL :: sl-type "*")
		(resTypeJS :: js-expr-type resTypeSL)
		(leftClauseSL :: fun (xSL :: sl-term lSL) -> sl-term resTypeSL)
		(leftClauseJS :: fun (xSL :: sl-term lSL) (xJS :: js-expr lJS xSL) -> js-expr resTypeSL (leftClauseSL xSL))
		(rightClauseSL :: fun (xSL :: sl-term rSL) -> sl-term resTypeSL)
		(rightClauseJS :: fun (xSL :: sl-term rSL) (xJS :: js-expr rJS xSL) -> js-expr resTypeSL (rightClauseSL xSL))
		= (js-expr
	(type resTypeJS)
	(spec (sl-term "case subject of (Left l r . x) -> (lc . x) (Right l r . x) -> (rc . x)"
		...
		(type "l" = lSL)
		(type "r" = rSL)
		(type "lc" (xSL :: sl-term lSL) = leftClauseSL xSL)
		(type "rc" (xSL :: sl-term rSL) = rightClauseSL xSL)
	))
	(impl [[
		(function(s) {
			if (s[0] == 'left') {
